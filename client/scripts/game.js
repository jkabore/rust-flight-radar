...
function redrawMap() {
  ...
  mapData.forEach((flight) => {
    ...
    if (index != -1) {
      ctx.drawImage(
        contactImages[index],
        flight.x * XFACTOR + MARGIN,
        flight.y * YFACTOR + MARGIN
      );
      ctx.font = "15px Arial";
      ctx.fillStyle = "white";
      ctx.fillText(
      ...
      ctx.fillText(
        "?",
        flight.x * XFACTOR + MARGIN,
        flight.y * YFACTOR + MARGIN + YFACTOR / 2
      );
    }
  });
}
...
function updateData() {
  ...
  request.done(function (response, textStatus, jqXHR) {
    console.log(
      "Info: received data from map server - " + textStatus,
      response
    );
  ...
  request.fail(function (jqXHR, textStatus, errorThrown) {
    console.log(
      "Error: failed to request data from map server - " + textStatus,
      errorThrown
    );
  });
